...
{
    public static readonly {{typeSymbol.Name}}[] Get{{typeSymbol.Name}}Array
        = new {{typeSymbol.Name}}[]{ {{string.Join(", ", enumMemberList.Select(x => x.NameSpace))}} };
...
{{string.Join(Environment.NewLine, enumMemberList.Select(x => $"\t\t\t{x.NameSpace} => {x.Name},"))}}
    ...
    public static string ToAlias(this {{typeSymbol.Name}} type)
    {
        return type switch
        {
